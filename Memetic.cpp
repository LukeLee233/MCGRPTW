...
//            ns.create_individual(mcgrp, ns.ns_indi);
        ...
        int randNo1, randNo2;    //The ID of parents who involve in cross overing
...
vector<int> Memetic::cal_loads(const MCGRP &mcgrp, const vector<vector<int>> &XRoutes)
{
    ...
    for (int i = 0; i < XRoutes.size(); ++i)   //each route
    {
        ...
        for (int j = 1; j < XRoutes[i].size() - 1; ++j)
            cumulative += mcgrp.inst_tasks[XRoutes[i][j]].demand;
        load_buffer.push_back(cumulative);
    }
    return load_buffer;
}
...
{
    //analyse the distribution of tasks, exclude dummy task
    Omitted_task.clear();
...
//		task which never appears
        if (task_distribution[i].empty())
            ...
            if (reserve_info.route == -1) {
                cout << "After route crossover, no duplicate task!\n";
            }
            else {
                for (auto route_id : task_distribution[i]) {
...
{
    My_Assert(sim == SIMILARITY::COST_DISTANCE, "Unknown similarity metric!");
...
bool
HighSpeedMemetic::repeated(const MCGRP &mcgrp, const double tested_value, const vector<int> &immigrant, SIMILARITY sim)
{
